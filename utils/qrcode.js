(function() {
      ...
      for (var row = 0; row < moduleCount; row++) {
        for (var col = 0; col < moduleCount - 6; col++) {
          if (qrCode.isDark(row, col) && !qrCode.isDark(row, col + 1) && qrCode.isDark(row, col + 2)
            && qrCode.isDark(row, col + 3) && qrCode.isDark(row, col + 4)
            && !qrCode.isDark(row, col + 5) && qrCode.isDark(row, col + 6)) {
            lostPoint += 40;
          }
        }
      }
      for (var col = 0; col < moduleCount; col++) {
        for (var row = 0; row < moduleCount - 6; row++) {
          if (qrCode.isDark(row, col) && !qrCode.isDark(row + 1, col) && qrCode.isDark(row + 2, col)
            && qrCode.isDark(row + 3, col) && qrCode.isDark(row + 4, col)
            && !qrCode.isDark(row + 5, col) && qrCode.isDark(row + 6, col)) {
            lostPoint += 40;
          }
        }
      }
  ...
  function getTypeNumber(text, errorCorrectLevel) {
    var length = encodeURI(text).toString().replace(/\%[0-9A-F]{2}/g, 'a').length;
    var type = 1, eclevel;
    for (; type < 40; type++) {
      var rsBlocks = QRRSBlock.getRSBlocks(type, errorCorrectLevel);
      var totalDataCount = 0;
      for (var i = 0; i < rsBlocks.length; i++) totalDataCount += rsBlocks[i].dataCount;
      eclevel = 4 + (length * 8 < totalDataCount * 8);
      if (length < Math.pow(2, (8 + 4 * (type < 10) + 2 * (type < 27) + 2 * (type < 40))) - 1 && eclevel > 4) break;
    }
    return type;
  }

  function generateQRModules(text, errorCorrectLevel) {
    if (typeof errorCorrectLevel === 'undefined') errorCorrectLevel = QRErrorCorrectLevel.M;
  ...
  function drawQRCanvas(canvasId, text, size, options) {
    return new Promise(function(resolve, reject) {
      options = options || {};
      var errorCorrectLevel = options.errorCorrectLevel || QRErrorCorrectLevel.M;
      ...
      var centerIcon = options.centerIcon || null;
      var centerIconSize = options.centerIconSize || 0.2;

      var modules;
      try {
        modules = generateQRModules(text, errorCorrectLevel);
      } catch (e) {
        reject(e);
        return;
      }
      ...
      query.select(canvasId)
        ...
        .exec(function(res) {
          ...
          try {
            ...
            if (centerIcon) {
              var iconSize = Math.floor(actualSize * centerIconSize);
              var iconX = (actualSize - iconSize) / 2;
              var iconY = (actualSize - iconSize) / 2;
              var padding = Math.floor(iconSize * 0.1);

              ctx.fillStyle = lightColor;
              ctx.fillRect(iconX - padding, iconY - padding, iconSize + padding * 2, iconSize + padding * 2);
              ctx.strokeStyle = lightColor;
              ctx.lineWidth = padding;
              ctx.strokeRect(iconX - padding / 2, iconY - padding / 2, iconSize + padding, iconSize + padding);

              if (typeof centerIcon === 'string') {
                var iconCanvas = canvas;
                var iconCtx = ctx;
                if (centerIcon.length <= 4) {
                  var fontSize = Math.floor(iconSize * 0.6);
                  iconCtx.fillStyle = darkColor;
                  iconCtx.font = 'bold ' + fontSize + 'px sans-serif';
                  iconCtx.textAlign = 'center';
                  iconCtx.textBaseline = 'middle';
                  iconCtx.fillText(centerIcon, iconX + iconSize / 2, iconY + iconSize / 2 + 2);
                }
              }
            }
            resolve({ width: actualSize, height: actualSize });
          } catch (err) {
            reject(err);
          }
        });
  ...
  module.exports = {
    generateQRModules: generateQRModules,
